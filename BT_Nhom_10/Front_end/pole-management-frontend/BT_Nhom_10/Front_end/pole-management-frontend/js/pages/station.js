const DEFAULT_STATIONS = [
  {
    ...
    connectors: [
      { name: "Cổng 1 - CCS2", status: "Sẵn sàng" },
      { name: "Cổng 2 - Type 2", status: "Đang sử dụng" }
    ]
  },
  {
    id: "ST002",
    ...
    createdAt: "2025-07-29T10:45:00",
    connectors: [
      { name: "Cổng 1 - CCS2", status: "Bảo trì" },
      { name: "Cổng 2 - CHAdeMO", status: "Sẵn sàng" },
      { name: "Cổng 3 - Type 2", status: "Sẵn sàng" }
    ]
  },
  {
    id: "ST003",
    ...
    createdAt: "2025-07-29T10:45:00",
    connectors: [{ name: "Cổng 1 - CCS2", status: "Ngưng hoạt động" }]
  }
];

const POLE_PRESETS = {
  "pole-1": [{ name: "Cổng 1 - Type 2", status: "Sẵn sàng" }],
  "pole-2": [
    { name: "Cổng 1 - CCS2", status: "Sẵn sàng" },
    { name: "Cổng 2 - Type 2", status: "Sẵn sàng" }
  ],
  "pole-3": [
    { name: "Cổng 1 - CCS2", status: "Sẵn sàng" },
    { name: "Cổng 2 - CHAdeMO", status: "Sẵn sàng" },
...
function getStatusFilterValue(status) {
  ...
  if (status === "Ngưng hoạt động") return "inactive";
  return "other";
}
...
function openCreateStationDrawer(station = null) {
  ...
  if (station) {
    ...
    if (els.createStatus) els.createStatus.value = station.status || "Đang hoạt động";
    ...
    if (els.createStatus) els.createStatus.value = "Đang hoạt động";
...
function toggleCreateStatus() {
  ...
  els.createStatus.value =
    getStatusFilterValue(els.createStatus.value) === "inactive" ? "Đang hoạt động" : "Ngưng hoạt động";
  updateCreateStatusToggle();
}

function stationHasActivePoles(station) {
  if (!station || !Array.isArray(station.connectors)) return false;
  return station.connectors.some((connector) => connector.status !== "NgÆ°ng hoáº¡t Ä‘á»™ng");
}
...
function toggleCreateStatus() {
  if (!els.createStatus) return;
  els.createStatus.value =
    els.createStatus.value === "NgÆ°ng hoáº¡t Ä‘á»™ng" ? "Äang hoáº¡t Ä‘á»™ng" : "NgÆ°ng hoáº¡t Ä‘á»™ng";
  updateCreateStatusToggle();
}
...
function applyDeactivatedStatus() {
  if (!els.createStatus) return;
  els.createStatus.value = "Ngưng hoạt động";
  updateCreateStatusToggle();
}
...
function toggleCreateStatus() {
  ...
  els.createStatus.value = currentValue === "inactive" ? "Đang hoạt động" : "Ngưng hoạt động";
  updateCreateStatusToggle();
}
...
function handleCreatePoleSelection() {
  ...
  if (!value) return;
  if (!selectedPoleValues.includes(value)) {
    selectedPoleValues = normalizeSelectedPoles([...selectedPoleValues, value]);
  }
...
function renderTable() {
  ...
  if (!filteredStations.length) {
    els.tableBody.innerHTML = `
      <tr>
        <td colspan="7" class="muted">Không tìm thấy trạm phù hợp.</td>
      </tr>
    `;
  ...
  els.stationPagination.innerHTML = `
    <button type="button" class="pagination-btn" data-page-nav="prev" aria-label="Previous page" ${prevDisabled}>
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        <path d="M15 5L8 12L15 19" stroke="currentColor" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
    </button>
    <div class="pagination-list">
      ${items
        .map((item) => {
          ...
          return `<button type="button" class="pagination-page${activeClass}" data-page="${item}" aria-label="Page ${item}">${item}</button>`;
        })
        .join("")}
    </div>
    <button type="button" class="pagination-btn" data-page-nav="next" aria-label="Next page" ${nextDisabled}>
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        <path d="M9 5L16 12L9 19" stroke="currentColor" stroke-width="2.8" stroke-linecap="round" stroke-linejoin="round"/>
      </svg>
    </button>
  `;
}
...
function pauseStation(stationId) {
  const station = findStationById(stationId);
  if (!station) return;
  station.status = "Ngưng hoạt động";
...
function deleteStation(stationId) {
  ...
  if (stationHasActivePoles(station)) {
    window.alert("Không thể xóa trạm đang có trụ hoạt động");
    return false;
  }
  ...
  applySearchFilter();
  showList();
  window.alert("Xóa trạm thành công");
  return true;
}
...
function confirmAction() {
  ...
  switch (currentAction) {
    case "update":
      ...
      break;
    case "pause":
      pauseStation(currentStation.id);
      break;
    case "delete":
      deleteStation(currentStation.id);
      break;
    default:
      break;
  }
...
function toggleCreateStatus() {
  ...
  els.createStatus.value = "Đang hoạt động";
  updateCreateStatusToggle();
}
...
function bindEvents() {
  ...
  if (els.createBtn && els.createBtn.tagName === "BUTTON") {
  ...
  if (els.deleteConfirmYesBtn) {
    els.deleteConfirmYesBtn.addEventListener("click", () => {
  ...
  if (els.deactivateStationConfirmBtn) {
    els.deactivateStationConfirmBtn.addEventListener("click", () => {
      applyDeactivatedStatus();
      closeDeactivateOverlay();
    });
  }
  ...
  if (els.actionConfirmBtn) {
    els.actionConfirmBtn.addEventListener("click", confirmAction);
  }
  ...
  if (els.createForm) {
    els.createForm.addEventListener("submit", (e) => {
      ...
      if (Number.isNaN(latNum) || Number.isNaN(lonNum)) {
        showCreateError("Vĩ độ / Kinh độ không hợp lệ.");
        return;
      }
      ...
      if (status === "Ngưng hoạt động") {
        connectors.forEach((connector) => {
          connector.status = "Ngưng hoạt động";
        });
      }

      if (editMode && editingStationId) {
        const station = findStationById(editingStationId);
        if (!station) {
          showCreateError("Trạm không tồn tại.");
          return;
        }
        ...
        if (
          ...
          showCreateError("Thông tin trạm đã tồn tại. Vui lòng thử lại.");
          return;
        }
        ...
        window.alert("Cập nhật trạm thành công");
        return;
      }

      const id = nextStationId();
      if (stations.some((s) => s.id === id)) {
        showCreateError("Mã trạm đã tồn tại. Vui lòng thử lại.");
        return;
      }

      if (isDuplicateStationData({ name, address, latitude: String(latNum), longitude: String(lonNum) })) {
        showCreateError("ThÃ´ng tin tráº¡m Ä‘Ã£ tá»“n táº¡i. Vui lÃ²ng thá»­ láº¡i.");
        return;
      }
...
const HISTORY_SESSIONS = [
  { id: "HS001", stationId: "ST001", stationName: "Station 1", connector: "Cổng 1 - CCS2", status: "completed", start: "2026-03-29T08:10:00", end: "2026-03-29T09:05:00", kwh: 34.2, duration: 55, cost: 18.4, note: "Phiên sạc hoàn tất bình thường." },
  { id: "HS002", stationId: "ST001", stationName: "Station 1", connector: "Cổng 2 - Type 2", status: "interrupted", start: "2026-03-29T13:20:00", end: "2026-03-29T13:54:00", kwh: 12.6, duration: 34, cost: 6.8, note: "Ngắt giữa chừng do người dùng dừng phiên." },
  { id: "HS003", stationId: "ST002", stationName: "Station 2", connector: "Cổng 1 - CCS2", status: "failed", start: "2026-03-28T10:12:00", end: "2026-03-28T10:19:00", kwh: 1.3, duration: 7, cost: 0.7, note: "Lỗi handshake giữa trạm và xe." },
  { id: "HS004", stationId: "ST002", stationName: "Station 2", connector: "Cổng 2 - CHAdeMO", status: "completed", start: "2026-03-27T09:40:00", end: "2026-03-27T10:35:00", kwh: 29.8, duration: 55, cost: 16.1, note: "Phiên sạc ổn định trong toàn bộ quá trình." },
  { id: "HS005", stationId: "ST001", stationName: "Station 1", connector: "Cổng 1 - CCS2", status: "completed", start: "2026-03-26T18:15:00", end: "2026-03-26T19:10:00", kwh: 31.5, duration: 55, cost: 17.3, note: "Mức tiêu thụ phù hợp lịch sử trung bình." },
  { id: "HS006", stationId: "ST003", stationName: "Station 3", connector: "Cổng 1 - CCS2", status: "interrupted", start: "2026-03-24T07:50:00", end: "2026-03-24T08:22:00", kwh: 10.1, duration: 32, cost: 5.5, note: "Bảo trì phát hiện đầu nối lỏng, cần kiểm tra lại." },
  { id: "HS007", stationId: "ST002", stationName: "Station 2", connector: "Cổng 3 - Type 2", status: "completed", start: "2026-03-22T15:05:00", end: "2026-03-22T16:00:00", kwh: 28.4, duration: 55, cost: 15.2, note: "Phiên sạc hoàn thành, không ghi nhận bất thường." },
  { id: "HS008", stationId: "ST001", stationName: "Station 1", connector: "Cổng 2 - Type 2", status: "completed", start: "2026-03-20T11:25:00", end: "2026-03-20T12:08:00", kwh: 19.6, duration: 43, cost: 10.4, note: "Tải trung bình, thời lượng dưới ngưỡng cảnh báo." },
  { id: "HS009", stationId: "ST002", stationName: "Station 2", connector: "Cổng 2 - CHAdeMO", status: "completed", start: "2026-03-17T20:10:00", end: "2026-03-17T21:12:00", kwh: 36.1, duration: 62, cost: 19.6, note: "Có đỉnh tải cao vào cuối phiên sạc." },
  { id: "HS010", stationId: "ST003", stationName: "Station 3", connector: "Cổng 1 - CCS2", status: "failed", start: "2026-03-15T06:30:00", end: "2026-03-15T06:38:00", kwh: 0.8, duration: 8, cost: 0.4, note: "Thiết bị dừng do cảm biến nhiệt độ." }
];
...
function initSecondarySectionToggles() {
  document.querySelectorAll(".sb-sec-toggle").forEach((toggle) => {
